...
bool callback_key_down(igl::opengl::glfw::Viewer& viewer, unsigned char key, int modifiers)
{
	if (key == '1')
	{
		...
		cout << "Vertex to Face Relations:" << endl;
		cout << "-------------------------" << endl;
		auto v_i = 0;
		for each (auto v_f in VF)
		{
			cout << "Vertex: " << v_i << "       " << "Adjacent Faces: ";
			for each (auto f_i in v_f)
			{
				cout << f_i << " ";
			}
			cout << endl;
	...
	if (key == '2')
	{
		...
		cout << "Vertex to Vertex Relations:" << endl;
		cout << "---------------------------" << endl;
		auto v_i = 0;
		for each (auto v_v in VV)
		{
			cout << "Vertex: " << v_i << "       " << "Adjacent Vertices: ";
			for each (auto i in v_v)
			{
				cout << i << " ";
			}
			cout << endl;
	...
	if (key == '3')
	{
		...
		int components = cid.maxCoeff() + 1;
		cout << "Connected Components:" << endl;
		cout << "---------------------" << endl;
		for (int i = 0; i < components; i++)
		{
			int faces = 0;
			for each (auto id in cid)
				if (i == id)
					faces++;
			cout << "Component Id: " << i + 1 << "       " << "Faces Count: " << faces << endl;
		}
	}
	...
	if (key == '4')
	{
		...
		Eigen::MatrixXd Vout, M, P;
		Eigen::MatrixXi Fout, FF;
		size_t inserted = 0, v_count = V.rows(), f_count = F.rows();
		// Setting M as described in the algorithm
		igl::barycenter(V, F, M);
		// Setting P as described in the algorithm
		igl::adjacency_list(F, VV);
		...
		for (size_t i = 0; i < v_count; i++)
		{
			...
			float a_n = (4.0f - 2.0f * cos(2.0f * PI / n)) / 9.0f;
			P.row(i) = (1.0f - a_n) * V.row(i) + (a_n / n) * P.row(i);
		}
		// Setting Vout to V' as described in the algorithm
		Vout.resize(M.rows() + P.rows(), M.cols());
		Vout << P, M;
		// Setting Fout to F' as described in the algorithm
		...
		for (size_t i = 0; i < f_count; i++)
		{
			...
			{
				if (FF(i, j) < 0)
				{   // There is no neighbor on the j'th edge of the triangle ->
					// Connecting the vertices of the j'th edge to the midpoint of the triangle
					Fout.row(inserted++) << F(i, j), F(i, (j + 1) % 3), V.rows() + i;
				}
				else if (i > FF(i, j))
				{   // There is a neighbor on the j'th edge of the triangle and we passed it ->
					// Each vertex of the j'th edge is connected to both the triangle's and its neighbor's midpoints
					Fout.row(inserted++) << FF(i, j) + V.rows(), F(i, (j + 1) % 3), V.rows() + i;
					Fout.row(inserted++) << F(i, j), FF(i, j) + V.rows(), V.rows() + i;
				}
			}
		}
		V = Vout; F = Fout;
